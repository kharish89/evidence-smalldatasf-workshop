...
```sql description
select min(opened) as min_date, max(opened) as max_date, count(case_id) as count from sf311.cases
```
...
```sql summary
select count(case_id) as total_cases, count(case_id) filter (where status = 'Open') as open_cases from sf311.cases
```
...
<LineChart
  title="{inputs.dimensions} Case Trend"
  data={trend}
  x=date
  y=cases
  yAxisTitle=cases
...
```sql cases
select responsible_agency, category, request_type, source from sf311.cases
```
